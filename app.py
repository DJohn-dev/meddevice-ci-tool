...
<style>
  ...
  .empty-box {
    ...
    color: #64748b; font-size: 13px; margin: 8px 0;
  }
  ...
  .tip-box {
    ...
    margin: 8px 0 16px 0; line-height: 1.6;
  }

  /* Welcome */
  .welcome {
    background: #f8fafc; border: 1px solid #e2e8f0;
    border-radius: 12px; padding: 48px; text-align: center; margin-top: 40px;
  }

  /* Suppress extra padding in metric */
  div[data-testid="metric-container"] { background: #f8fafc; border-radius: 8px; }

  /* Streamlit tab active color */
...
def empty_state(msg, search_url=None, search_label=None):
    link = f'<br><a href="{search_url}" target="_blank" style="color:#3b82f6">{search_label} →</a>' if search_url else ""
...
def render_fei(data, num):
    ...
    if not ests:
        ...
        empty_state(
            "No FDA establishment records found for this name.<br>"
            "FDA stores names in ALL CAPS and may truncate 'Inc.' — try searching directly:",
            f"https://www.accessdata.fda.gov/scripts/cdrh/cfdocs/cfrl/rl.cfm?start_search=1&establishment_name={encoded}",
...
def render_510k(data, num):
    ...
    p_items= dpma.get("items", [])

    metric_row([
        ("Total 510(k)s",  f"{d510.get('total', 0):,}"),
        ("Shown",          len(items)),
        ("PMA Approvals",  len(p_items)),
    ])
    ...
    with tab1:
        if items:
            ...
            name = st.session_state.get("company_name", "")
            empty_state(
                "No 510(k) records matched. FDA may store the applicant name differently.",
...
def render_maude(data, num):
    ...
    if not items:
        ...
        encoded = name.replace(" ", "+")
        empty_state(
            "No MAUDE records matched. The manufacturer name in MAUDE may differ from the company name you entered.",
            f"https://www.accessdata.fda.gov/scripts/cdrh/cfdocs/cfmaude/search.cfm",
    ...
    metric_row([
        ...
        ("Shown (recent 25)",   len(items)),
    ])
    df = pd.DataFrame([{
        "Date":        fmt_date(i["date"]),
        "Event Type":  i["event_type"],
        "Device":      i["brand_name"][:45],
...
def render_recalls(data, num):
    ...
    if not items:
        empty_state(
            "No recall records found for this company name.",
...
def render_trials(data, num):
    ...
    if not items:
        ...
        encoded = name.replace(" ", "+")
        empty_state(
            "No clinical trials found for this sponsor name.",
            f"https://clinicaltrials.gov/search?spons={encoded}",
...
def render_payments(data, num):
    ...
    if d.get("error"):
        ...
        tip(f'Search Open Payments directly: '
            f'<a href="https://openpaymentsdata.cms.gov/search?searchType=Company&Name={encoded}" '
            f'target="_blank">openpaymentsdata.cms.gov →</a>')
        return
    ...
    with tab1:
        if by_year:
            fig = go.Figure(go.Bar(
                x=list(by_year.keys()), y=list(by_year.values()),
                marker_color="#3b82f6",
    ...
    with tab2:
        if by_type:
            ...
            st.plotly_chart(fig, use_container_width=True)
            df_t = pd.DataFrame([
                {"Payment Type": k, "Total": fmt_usd(v), "_raw": v}
                for k, v in by_type.items()
            ]).sort_values("_raw", ascending=False).drop(columns="_raw")
            st.dataframe(df_t, use_container_width=True, hide_index=True)

    with tab3:
        us_states = {k: v for k, v in by_state.items()
                     if k and len(k) == 2 and k.isalpha()
                     and k not in ("XX", "PR", "GU", "VI", "AS", "MP")}
        if us_states:
            ...
            st.plotly_chart(fig, use_container_width=True)
        df_s = pd.DataFrame([
            {"State": k, "Total Paid": fmt_usd(v), "_raw": v}
            for k, v in by_state.items() if k and k != "Unknown"
        ]).sort_values("_raw", ascending=False).drop(columns="_raw").head(20)
        if not df_s.empty:
    ...
    with tab4:
        if top_kols:
            rows = [{"Physician": n, "Specialty": i["specialty"][:50] or "—",
                     ...
                     "Transactions": i["count"], "_raw": i["total"]}
                    for n, i in top_kols]
            df_k = pd.DataFrame(rows).sort_values("_raw", ascending=False).drop(columns="_raw")
            st.dataframe(df_k, use_container_width=True, hide_index=True)

            names  = [r["Physician"]    for r in rows[:15]]
            totals = [r["_raw"]         for r in sorted(rows, key=lambda x: x["_raw"], reverse=True)[:15]]
            fig = go.Figure(go.Bar(
                x=totals[::-1], y=names[::-1], orientation="h",
                marker_color="#3b82f6",
                text=[fmt_usd(v) for v in totals[::-1]], textposition="outside",
            ))
...
def render_spending(data, num):
    ...
    with tab1:
        if contracts:
            df = pd.DataFrame([{"Award ID": c.get("Award ID","—"),
                "Amount": fmt_usd(c.get("Award Amount",0)),
                "Agency": str(c.get("Awarding Agency","—"))[:50],
                "Type": c.get("Award Type","—"), "Start": c.get("Start Date","—")}
                for c in contracts])
            st.dataframe(df, use_container_width=True, hide_index=True)
        else:
...
def render_company(company_name, company_type, cik, sections, data):
    ...
    for sec in section_order:
        ...
        # Skip sec for private, spending for public
...
def company_form(prefix, label):
    ...
    if ctype == "public":
        ...
        if not cik:
            st.caption("[Find CIK on EDGAR →](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&company=&CIK=&type=10-K&dateb=&owner=include&count=10&search_text=&action=getcompany)")
...
def main():
    ...
    # Store company name in session state for use in empty-state links
